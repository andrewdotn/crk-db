...
export default function createHomographKey(lemma, keys) {
  ...
  const baseKey      = createKey(lemma);
  let   homographNum = 1;
  let   homographKey = baseKey;
  ...
  const setHomographNum = () => {
    ...
    const existingEntry = keys.get(baseKey + homographNum) ?? keys.get(baseKey);
    ...
    homographNum++;
    homographKey = baseKey + homographNum;
    setHomographNum();
