...
// Stores a running unique list of all the known entry keys
const keys = new Set;
...
function processNote(noteText, entry, sense) {
  ...
  if (isCrossReference) {
    entriesImproved++;
    return processCrossReference(noteText, entry, sense);
  }

  const isLiteralDefinition = /^lit[:.]{1,2}|literally/u.test(noteText);
  if (isLiteralDefinition) {
    entriesImproved++;
    return processLiteralDefinition(noteText, entry, sense);
  }

  const isLatinTerm = /^Lt[:.]|Latin/u.test(noteText);
  if (isLatinTerm) {
    entriesImproved++;
    return processLatinTerm(noteText, entry, sense);
  }

  const isUsageNote = /:$/u.test(noteText);
  if (isUsageNote) {
    entriesImproved++;
    return processUsageNote(noteText, entry, sense);
  }
...
export default async function convertCW(inputPath, outputPath) {
  ...
  console.info(`Entries improved: ${entriesImproved}`);
