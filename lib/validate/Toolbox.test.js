...
describe(`Toolbox database`, function() {
  ...
  it(`does not contain unwanted character sequences`, function() {
  ...
  it(`does not contain data other than "new" in the \\new field`, function() {
    for (const entry of this.toolboxEntries) {
      const line = entry.getLine(`new`);
      if (typeof line !== `undefined`) {
        if (line.text !== `new`) console.log(entry.sro);
        expect(line.text).to.equal(`new`);
      }
    }
  });
